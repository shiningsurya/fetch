...
Original implementation from FDMT Zackay & Ofek (2014)
...
cdef class FDMT:
    ...
    def Bowtie_maxdelay (self, np.ndarray[cf_t, ndim=2] Image, Py_ssize_t maxdt):
        '''
        ...
        ###
        cdef Py_ssize_t ddnsamps           = self.T - maxdt
        cdef np.ndarray[cf_t, ndim=2] bt   = np.zeros ((maxdt, ddnsamps), dtype=f_t)
        for i in range (maxdt):
            bt[i] = self.State [0, i, :ddnsamps]
        return bt
    ...
    def Bowtie_delays (self, np.ndarray[cf_t, ndim=2] Image, np.ndarray[np.uint32_t, ndim=1] delays):
        ...
        Returns
        -------
        Bowtie : array with shape=(delays.size, NSAMP)
        '''
        ...
        cdef np.ndarray fbt                = self.Bowtie_maxdelay (Image, maxdt)
        # dmrange selection
